...
class Knitting_Needles extends Component {
    state = {
        templateData: [],
        language: "",
        seoTitle: "",
    ...
    componentDidMount() {
        ...
        if (lang) {
            this.setState(
                ...
                () => {
                    getAPI(`template/getMenuTemplates/10?lang=${this.state.language}`)
                        .then((res) => {
                            const { status, data } = res.data;
                            if (status === 1) {
                                if (data.length > 0) {
                                    if (data[0].adminMenuUid === 10 && data[0].type === "topBanner") {
                                        this.setState(
                                            {
...
export async function getStaticProps() {
    ...
    const getPageProps = async (lang) => {
        const res = await fetch(`${Baseurl}template/getMenuTemplates/10?lang=${lang}`)
...
// export default withRouter(withTranslation()(Knitting_Needles));
...
export default withRouter(Knitting_Needles) 