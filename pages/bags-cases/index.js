...
class BagsCases extends Component {
    state = {
        templateData: [],
        language: "",
        seoTitle: "",
    ...
    //             },
    //             () => {
    //                 getAPI(`template/getMenuTemplates/13?lang=${this.state.language}`)
    //                     .then((res) => {
    //                         const { status, data } = res.data;
    //                         if (status === 1) {
    //                             if (data.length > 0) {
    //                                 if (data[0].adminMenuUid === 13 && data[0].type === "topBanner") {
    //                                     this.setState(
    //                                         {
...
export async function getStaticProps() {
    ...
    const getPageProps = async (lang) => {
        const res = await fetch(`${Baseurl}template/getMenuTemplates/13?lang=${lang}`)
...
// export default withRouter(withTranslation()(BagsCases));
...
export default withRouter(BagsCases)
